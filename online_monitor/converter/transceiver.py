...
class Transceiver(multiprocessing.Process):
    ...
    def recv_commands(self):
        commands = []
        if self.backend_socket_type == zmq.DEALER:  # Check for bidirectional communication
            for actual_backend in self.backends:
                try:
                    command = actual_backend[1].recv(zmq.NOBLOCK)  # Check if command was send
    ...
    def setup_transceiver(self):
        ''' Method can be defined to setup transceiver specific parameters (e.g. bidirectional communication)'''
        pass
    ...
    def handle_command(self, commands):
        ''' Command received in bidirectional communication mode from a receiver.'''
